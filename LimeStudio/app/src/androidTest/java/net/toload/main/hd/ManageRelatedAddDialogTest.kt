...
open class ManageRelatedAddDialogTest {
    @Test
    fun testManageRelatedAddDialogClassExists() {
        try {
            var cls: Class<*> = Class.forName("net.toload.main.hd.ui.dialog.ManageRelatedAddSheet")
            assertNotNull(cls)
    ...
    @Test
    fun testValidationAndControllerAddRelatedApis() {
        var dialog: Class<*> = Class.forName("net.toload.main.hd.ui.dialog.ManageRelatedAddSheet")
        ...
        var hasAddRelated: Boolean = false
        for (m in ctrl.methods) {
            var n: String = m.name.lowercase()
            if ((n.contains("add") && n.contains("related"))) {
                hasAddRelated = true
                break
            }
        }
    ...
    fun testSheetLayoutScrollsWhenImeIsVisible() {
        ...
        assertEquals("ManageRelatedAddSheet root should scroll above the soft keyboard", "androidx.core.widget.NestedScrollView", getRootTagName(context, R.layout.sheet_manage_related_add))
    }
    @Test
    fun testSheetMatchesIosRowEditorControls() {
        var context: Context = InstrumentationRegistry.getInstrumentation().getTargetContext()
        assertEquals("新增資料列", context.getString(R.string.manage_related_dialog_add))
        assertNotEquals("Related add sheet should expose a cancel button", 0, R.id.btn_cancel)
        assertNotEquals("Related add sheet should expose an editable score field", 0, R.id.edt_score)
    }

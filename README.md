...
Sam 版本以 Android 21 至 36 相容性、萊姆輸入法核心穩定性、內建語音輸入、Emoji 候選與資料庫管理流程為主要維護重點。
...
- 支援萊姆鍵盤內建語音輸入；若未授權麥克風，仍可改用系統或 Google 語音輸入。
- 設定頁已改為新版導覽與資料庫管理流程，並集中管理主要界面文字資源。
- KeePass 頁面目前已接入設定導覽，資料庫建立、匯入、解鎖與密碼填入流程仍在後續實作。
...
| `RECORD_AUDIO` | 使用萊姆鍵盤內建語音輸入時存取麥克風；若不使用可不授權 |
...
萊姆鍵盤可直接使用內建語音輸入。第一次使用時請在設定頁允許麥克風權限；若略過或拒絕授權，仍可改用系統已安裝的語音輸入服務。

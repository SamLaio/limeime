...
Lime-Sam 版本以 Android 21 至 36 相容性、輸入法核心穩定性、內建語音輸入、Emoji 候選、KeePass 與資料庫管理流程為主要維護重點。
...
- 更新行列10資料庫，並改用哈哈倉頡 `.limedb` 取代舊倉頡四資料庫。
- 匯入 `.limedb` 時會對齊來源與目標欄位，避免不同資料庫 schema 造成匯入失敗。
- 支援候選列 Emoji 按鈕、Emoji 搜尋與內建 Emoji 資料表。
- 支援 Lime-Sam 鍵盤內建語音輸入；若未授權麥克風，仍可改用系統或 Google 語音輸入。
- 設定頁已改為新版導覽、資料庫管理流程與深色模式設計系統，並集中管理主要界面文字資源。
- KeePass 已接入設定頁、鍵盤流程、Autofill 與 Android Credential Provider；支援本機檔案、WebDAV、FTP、KDBX 開啟/瀏覽/新增/刪除、差分同步、本機 KDBX 暫存與加密 entry cache。
- KeePass 鍵盤可從長按空白鍵的輸入法選單進入，搜尋並選取項目後，以下方鍵盤式按鈕輸入使用者名稱、密碼、網址、備註，以及持卡人、號碼、過期、CVV 等指定卡片欄位；卡片欄位不參與搜尋。
...
| `VIBRATE` | 按鍵時提供觸覺震動回饋 |
| `INTERNET` | 從伺服器下載使用者選擇的輸入法碼表，並支援 KeePass WebDAV/FTP 遠端資料庫 |
| `ACCESS_NETWORK_STATE` | 下載或遠端同步前檢查網路是否可用，避免無效連線嘗試 |
| `RECORD_AUDIO` | 使用 Lime-Sam 鍵盤內建語音輸入時存取麥克風；若不使用可不授權 |
...
Lime-Sam 鍵盤可直接使用內建語音輸入。第一次使用時請在設定頁允許麥克風權限；若略過或拒絕授權，仍可改用系統已安裝的語音輸入服務。

### KeePass

設定頁的 KeePass 區塊可設定資料庫、金鑰與密碼。資料庫支援本機檔案、WebDAV 與 FTP，遠端資料庫會先建立本機 KDBX 暫存檔；app 內瀏覽與鍵盤搜尋使用本機 entry cache，加速開啟。

entry cache 內的密碼與指定卡片欄位會以資料庫密碼、金鑰或密碼加金鑰衍生出的金鑰加密。只有在已解鎖並選取項目後，才會解密該項目的密碼、CVV 等敏感欄位並顯示於 KeePass 鍵盤按鈕。同步時以本機暫存 KDBX 做 base/local/remote 差分合併，不會把 entry cache 的加密 payload 寫回 KDBX。
